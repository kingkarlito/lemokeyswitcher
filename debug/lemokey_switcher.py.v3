...
def via_read_one_keycode(dev, report_size: int, layer: int, row: int, col: int) -> int | None:
    """Read back a single keycode to verify a write succeeded."""
    via_send(dev, [VIA_DYNAMIC_KEYMAP_GET_KEYCODE, layer, row, col], report_size)
    resp = via_read(dev, report_size, timeout_ms=200)
...
def upload_layout_json(dev, layout_json: dict, report_size: int, status_cb=None):
    """
    Push a VIA layout JSON to the keyboard using the correct report size.

    Lemokey X4 matrix: 6 rows x 16 cols = 96 keys per layer (flat array).
    flat_index N  ->  row = N // MATRIX_COLS,  col = N % MATRIX_COLS

    Protocol: VIA cmd 0x07 (id_dynamic_keymap_set_keycode)
      [0x07, layer, row, col, keycode_high, keycode_low]

    We write all layers then do a spot-check read-back on a few keys to
    confirm the firmware actually accepted the writes.
    """
    ...
    total = sum(len(layer) for layer in layers if isinstance(layer, list))
    done = 0
    unknown = []
    ...
    for layer_idx, layer in enumerate(layers):
        ...
        for flat_idx, keycode_str in enumerate(layer):
            ...
            via_send(dev, payload, report_size)
            # Small delay to avoid overflowing the firmware's HID receive buffer.
            # 2ms * 384 keys = ~0.8s total which is acceptable.
            time.sleep(0.002)
            done += 1
            if status_cb and done % 30 == 0:
                pct = int(done / total * 100)
    ...
    # If readback matches what we wrote, the upload is confirmed working.
    # If it doesn't match, report size was probably still wrong.
    verified = False
    first_layer = layout_json["layers"][0]
    for flat_idx, keycode_str in enumerate(first_layer):
        if keycode_str in ("KC_NO", "KC_TRNS"):
            continue
        expected, _ = via_encode_keycode(keycode_str)
        row = flat_idx // MATRIX_COLS
        col = flat_idx % MATRIX_COLS
        ...
        if actual is not None:
            if actual == expected:
                verified = True
                print(f"[verify] OK  layer=0 row={row} col={col}  "
                      f"expected={expected:#06x} got={actual:#06x}  ({keycode_str})")
            else:
                print(f"[verify] MISMATCH  layer=0 row={row} col={col}  "
                      f"expected={expected:#06x} got={actual:#06x}  ({keycode_str})")
                if status_cb:
                    status_cb(
                        f"WARNING: key readback mismatch — expected {expected:#06x}, "
                        f"got {actual:#06x}. Try reconnecting."
                    )
    ...
    if status_cb:
        if verified:
            ...
            status_cb("Upload sent (could not verify readback).")
    return True
